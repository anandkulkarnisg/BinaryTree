...
void doInOrderTraversal(const BinaryTreeNode<int>* node, unordered_set<const int*>& hashSet)
{
  if(!node)
    return;
  doInOrderTraversal(node->m_leftChild, hashSet);
  hashSet.insert(&node->m_data);
  doInOrderTraversal(node->m_rightChild, hashSet);
}

void getNonPeripheryItems(const BinaryTreeNode<int>* node, unordered_set<const int*>& hashSet)
{
  // step1 : populate hashSet with address of all items in the BT [ O(n) complexity ].
  doInOrderTraversal(node, hashSet);

  // step 2 : get all the boundary items from BT. [ O(n) complexity again ]. 
  unordered_set<const int*> boundaryHashSet;
  getPeripheryItems(node, boundaryHashSet);

  // Final step : delete all those in hashSet which are in boundaryHashSet.Remaining items are non periphery items.
  for(const auto& iter : boundaryHashSet)
    hashSet.erase(iter);
}
...
int main(int argc, char* argv[])
{
  ...
  for(const auto& testCase : testCases)
  {
    ...
    getNonPeripheryItems(tree.getRoot(), hashSet);
    if(!hashSet.size())
      cout<<"Empty!!"<<endl;
    else
    {
